...
namespace InvoiceTotal
{
    public partial class frmInvoiceTotal : Form
	{
        ...
        // list variable add
        List<decimal> invoiceTotalList = new List<decimal>();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (subtotal > 0 && subtotal < 10000)
                    {
                        ...
                        // array add total and update index
		...
		private void btnExit_Click(object sender, EventArgs e)
		{
            ...
            foreach(decimal total in invoiceTotalsArray)
            {
                if (total != 0)
                {
                    message += totalIndex.ToString("c") + "\n";
                }
            }
            MessageBox.Show(message, "Order Totals - Array");


            // list display message add
            invoiceTotalList.Sort();
            message = "";
            
            foreach (decimal total in invoiceTotalList)
            {
                if (total != 0)
                {
