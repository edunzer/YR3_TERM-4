...
namespace CustomerMaintenance
{
    public partial class frmAddVehicle : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else if (chkCar.Checked)
            {
                if (IsValidData())
                {
                    vehicle = new Car(txtVin.Text, txtDescription.Text, txtPurchasePrice.Text, txtNumberOfDoors.Text);
                    this.Close();
                }
            }
            else if (chkTruck.Checked)
            {
                if (IsValidData())
                {
                    vehicle = new Truck(txtVin.Text, txtDescription.Text, txtPurchasePrice.Text, txtBedSize.Text);
                    this.Close();
                }
            }
            else
